function hjsCalendar(e) {
	...
	function recogerJSON() {//Función en la que llamamos al PHP que crea el JSON de las visitas programadas
	...
	function parsearFecha(json, año, mes, dia, hora) {//Función que recorre los objetos del json para ver las horas disponibles 

		const fechaComparar = new Date(año, mes, dia, hora);

		// Recorrer el JSON y comparar las fechas y horas
		for (const key in json) {//recorre cada objeto dle JSON
			if (json.hasOwnProperty(key)) {
				const elemento = json[key];//recorre cada elemento del objeto que hay en la iteración del JSON
				...
				if(elemento.trabajador == window.medicoSeleccionado){
					...
					// Comparar las fechas
					if (fechaComparar.getTime() === fechaJson.getTime()) {//compara fechas
...
`;
	...
	function m(e, t) {//Función que agrega los domingos a un array
		let n = [],
			i = new Date(e, t, 1);
	...
	function u() {//Función importante que hace que el calendario sea dinámico
		recogerJSON();
		Date.now();
		...
		n.innerHTML = h,
			function e() {//Función que crea los días en el calendario
					...
					e.addEventListener("click", i => {
						if (-1 !== n.indexOf(Number(e.textContent)));
							...
							let d = e.innerHTML,
								...
								u = "",
								f = "";
							for (let b = 8; b < 14; b++) {//Bucle que crea y habilita o deshabilita las horas en el calendario
								let coincide = false;
